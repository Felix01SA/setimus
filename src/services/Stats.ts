...
export class Stats {
    ...
    async getTotalStats() {
        const totalStatsObj = {
            ...
            TOTAL_ACTIVE_USERS: await prisma.user.count(),
            TOTAL_COMMANDS: await prisma.stat.groupBy({
                by: ['type'],
                _count: { _all: true },
            }),
        }
    ...
    async getLastInteraction() {
        const lastInteraction = await prisma.stat.groupBy({
            by: ['type', 'createdAt'],
     ...
     */
    sumStats(
        stats1: StatPerInterval,
        stats2: StatPerInterval
    ): StatPerInterval {
        const allDays = [
    ...
    async getPidUsage() {
        ...
        return {
            ...
            memory: {
                ...
                percentage: (
                    (pidUsage.memory / osu.mem.totalMem()) *
                    100
                ).toFixed(1),
            },
        }
    ...
    async registerDailyStats() {
        ...
        for (const type of Object.keys(totalStats)) {
            const value = JSON.stringify(
                totalStats[type as keyof typeof totalStats]
            )
